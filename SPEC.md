...
You are helping a developer integrate the xAI Realtime Voice API into their application. This is a public API — the developer is working outside of xAI and accessing it via their own API key in [console.x.ai](https://console.x.ai). The session configuration below reflects settings they configured in the xAI console playground.
...
## 0. Discovery Questions (study before any code)

Pay attention to the following definitions:

1. **Language / platform** — Build with [Bun](https://bun.com/) (Typescript).

2. **API key** — I already have an xAI API key with the Voice endpoint enabled.

3. **Existing project** — I am integrating this into an existing app (web app).

4. **Audio I/O** — Audio will be captured and played back by the browser MediaStream.

5. **Framework** — I am using Svelte.

6. **UI components** — For the frontend, use [shadcn/ui](https://ui.shadcn.com/) to scaffold a polished UI quickly.

7. **LiveKit** will not be used, I'm building a simple 1:1 browser voice agent — the direct (Bun) WebSocket approach has fewer moving parts, lower latency (no middleman), and deploys as a single app.
...
**Server-side** (Bun): pass as a Bearer token in the `Authorization` header.
        ...
        "parameters": {
          ...
          "properties": {
            ...
            "department": {
              ...
              "description": "Department to escalate to (e.g., billing, technical, management)"
...
The session above includes these custom function tools. When the assistant calls a tool, you'll receive a `response.function_call_arguments.done` event. Execute the function and send back the result:

- **`lookup_order`**(order_id, customer_email) — Look up order details and status

- **`track_shipment`**(order_id) — Get real-time shipping status and estimated delivery

- **`initiate_return`**(order_id, items, reason, refund_or_exchange) — Start a return or exchange process

- **`escalate_to_specialist`**(department, issue_summary, customer_contact) — Transfer the case to a specialized support team
...
│  ├─ VAD Threshold: Slider (0.1–0.9, step 0.05)                       │

│  ├─ Silence Duration: Slider (0–10000ms)                             │

│  ├─ Prefix Padding: Slider (0–10000ms)                               │
...
The user needs proof that their microphone is working before they start speaking. A visual element tied to real-time mic input level (RMS amplitude) provides this. When the user is silent it's still; when they speak it responds. This replaces the need for a "test your mic" step.
...
Settings are accessible at any time, but changes apply on the next connection. If a session is active, warn that changes won't take effect until they stop and restart.
...
async def entrypoint(ctx: JobContext):
    ...
    model = openai.realtime.RealtimeModel(
        ...
        base_url="wss://api.x.ai/v1/realtime?model=grok-voice-think-fast-1.0",
